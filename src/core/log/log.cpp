...
constexpr const char* m_loggerNAME = "nntoolkit_logger";

constexpr const char* LOG_PATTERN = "%Y-%m-%d %H:%M:%S.%f; %^%8l%$; thread %t; %v";

spdlog::sink_ptr CreateStderrSink(gamma::log::LoggingSettings::SyncMode syncMode, gamma::log::LOG_LEVEL level)
{
    spdlog::sink_ptr sink;
    if (syncMode == gamma::log::LoggingSettings::SyncMode::Sync)
